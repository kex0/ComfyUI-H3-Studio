...
# One 17-frame H3 group (5 latent steps). Clip 1 I2VA/FL2VA holds the first-frame
# still across this group; packing it as the loop landing produces a pose pause.
LOOP_OPENING_SKIP_STEPS = 5


def loop_opening_skip_steps(source_latent_t, context_frames):
    """Phase-0 skip past clip 1's I2VA still-hold, or 0 if the clip is too short."""
    if context_frames not in CONTEXT_TO_STEPS:
        raise ValueError(f"context_frames must be one of {sorted(CONTEXT_TO_STEPS)}")
    ctx_steps = CONTEXT_TO_STEPS[int(context_frames)]
    skip = LOOP_OPENING_SKIP_STEPS
    if skip + ctx_steps > int(source_latent_t):
        return 0
    return skip


def loop_wrap_start_frames(source_latent_t, context_frames):
    """Clip 1 frames to drop so the stitched file starts at the packed opening.

    That opening is the same window packed onto the Loop clip's tail. After
    stitch, the first and last ``context_frames`` are two takes of that window
    (real clip 1 vs Loop-generated); wrap overlap-adds them.
    """
    return pixel_frames(loop_opening_skip_steps(source_latent_t, context_frames))


def loop_end_keyframe_offsets(frame_count, context_frames, source_latent_t=None):
    """Pack clip 1's opening after the I2VA hold as the Loop clip's end context.

    The first ``LOOP_OPENING_SKIP_STEPS`` are the still-hold. The next
    ``context_frames`` (canonical head grid) are placed on the last
    ``context_frames`` of the loop clip. Stitch keeps that generated copy and
    overlap-adds it with clip 1 starting at ``wrap_start_frames``.
    """
    if context_frames not in CONTEXT_TO_STEPS:
        raise ValueError(f"context_frames must be one of {sorted(CONTEXT_TO_STEPS)}")
    ctx_steps = CONTEXT_TO_STEPS[int(context_frames)]
    ...
    if actual >= frame_count:
        raise ValueError("loop end context does not fit in the target clip")
    if source_latent_t is None:
        skip = LOOP_OPENING_SKIP_STEPS
    else:
        skip = loop_opening_skip_steps(source_latent_t, context_frames)
