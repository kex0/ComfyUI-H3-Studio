...
def crop_transform_frames(images, xform):
    """Crop a frame batch with an already-tracked transform. Allocates only this slice."""
    cw, ch = xform["canvas"]
    boxes = xform["boxes"]
    n = int(images.shape[0])
    if n != len(boxes):
        raise ValueError(
            f"h3_facerefine: crop slice has {n} frames but transform has {len(boxes)} boxes"
        )
    crops = torch.empty((n, int(ch), int(cw), 3), dtype=images.dtype, device=images.device)
    for i in range(n):
        crops[i:i + 1] = _affine_crop(images[i:i + 1], boxes[i], cw, ch)
    return crops
...
def _gaussian_blur_hwc(img: torch.Tensor, sigma: float) -> torch.Tensor:
    """Separable Gaussian blur on [N,H,W,C]. sigma in pixels of this tensor."""
    import torch.nn.functional as F

    if sigma <= 0.2:
        return img
    _n, h, w, c = img.shape
    k = max(3, int(round(float(sigma) * 6.0)) | 1)
    shortest = min(h, w)
    if shortest <= k:
        k = max(3, int(shortest / 2) | 1)
    x = torch.arange(k, device=img.device, dtype=torch.float32) - k // 2
    ...
    t = F.conv2d(F.pad(t, (pad, pad, 0, 0), mode="replicate"), gh, groups=c)
    t = F.conv2d(F.pad(t, (0, 0, pad, pad), mode="replicate"), gv, groups=c)
    return t.movedim(1, -1)


def _dilate_mask_hwc(mask: torch.Tensor, px: int) -> torch.Tensor:
    import torch.nn.functional as F

    if px <= 0:
        return mask
    k = 2 * int(px) + 1
    t = F.max_pool2d(mask.movedim(-1, 1), k, stride=1, padding=k // 2)
    return t.movedim(1, -1)
...
class H3FaceTrackCrop:
    ...
    def run(self, images, detector, confidence, crop_factor, canvas_width, canvas_height,
        ...
        sz_before = _jit(raw_sz)

        # Canvas aspect is the crop aspect. Auto modes are square so a 16:9 source
        # is not planned as 16:9 and then squashed into 768x768.
        if canvas_mode != "manual":
            need = float(min(raw_sz.max() * crop_factor, H))
            snapped = int(np.ceil(need / 32.0) * 32)
            if canvas_mode == "auto_capped_768":
                snapped = min(snapped, 768)
            snapped = max(128, min(snapped, 1344))
            if snapped != canvas_height or snapped != canvas_width:
        ...
        box_cx = np.array([b[0] + b[2] * 0.5 for b in boxes], dtype=np.float64)
        box_cy = np.array([b[1] + b[3] * 0.5 for b in boxes], dtype=np.float64)
        box_h = np.array([b[3] for b in boxes], dtype=np.float64)
        jit_after = (_jit(box_cx) + _jit(box_cy)) / 2.0
        sz_after = _jit(box_h)
        ...
        transform = {
            ...
            "via_body": [bool(v) for v in via_body],
            "face_h": [float(v) for v in sz_s],
            "face_w": [float(v) for v in fw_s],
            "face_rect": [
                face_rect_in_canvas(
                    b, cx_s[i], cy_s[i], fw_s[i], sz_s[i],
            ...
            "crop_plan": crop_plan,
            "shot_breaks": [bool(v) for v in breaks],
            "maskvid_breaks": [bool(v) for v in maskvid_breaks],
            "maskvid_cut": "center",
        }
        ...
        report = (
            ...
            f"face height  min={raw_sz.min():.0f}px  mean={raw_sz.mean():.0f}px  max={raw_sz.max():.0f}px\n"
            ...
            f"jitter centre {jit_before:.2f} -> {jit_after:.2f} px/frame"
        ...
        if crops is None:
            crops = preview = torch.zeros((1, 8, 8, 3), dtype=torch.float32)
...
class H3FaceStitch:
        ...
        return {
            "required": {
                ...
                "detail_match": ("FLOAT", {"default": 1.0, "min": 0.0, "max": 1.0, "step": 0.05,
                    "tooltip": "Soften H3's extra sharpness to match the source around the face. "
                               "Does not copy original face pixels (that ghosts when H3 drifts). "
                               "0 keeps H3's extra crunch."}),
    ...
    def run(self, base_images, refined_crops, transform, paste_region, mask_dilation, feather,
            colour_match, blend, detail_match, undetected_frames="fade_out", masks=None,
        ...
        for c0 in range(0, B, chunk):
            ...
            # Match surround sharpness by blurring H3. Do not copy source-face
            # pixels: high denoise drifts the head and that mix ghosts.
            if float(detail_match) > 0.0:
                patch_low = _gaussian_blur_hwc(patch, 1.25)
                base_low = _gaussian_blur_hwc(base, 1.25)
                luma = torch.tensor([0.299, 0.587, 0.114], device=dev, dtype=torch.float32)
                y_p = ((patch - patch_low) * luma).sum(dim=-1, keepdim=True)
                y_b = ((base - base_low) * luma).sum(dim=-1, keepdim=True)
                ring = (_dilate_mask_hwc(m, 12) - m).clamp(0, 1)
                interior = m
                e_h3 = (y_p.square() * interior).sum(dim=(1, 2, 3)) / interior.sum(
                    dim=(1, 2, 3)
                ).clamp_min(1e-6)
                e_src = (y_b.square() * ring).sum(dim=(1, 2, 3)) / ring.sum(
                    dim=(1, 2, 3)
                ).clamp_min(1e-6)
                amt = (float(detail_match) * (e_h3 - e_src).clamp_min(0.0) / e_h3.clamp_min(1e-8))
                amt = amt.clamp(0.0, 1.0).view(n, 1, 1, 1)
                ring_ok = ring.sum(dim=(1, 2, 3)).view(n, 1, 1, 1) > 64.0
                mag = torch.tensor(
                    [float(ch) / max(float(boxes[i][3]), 1.0) for i in range(c0, c1)],
                    device=dev, dtype=torch.float32,
                ).view(n, 1, 1, 1)
                fallback = (float(detail_match) * ((mag - 1.0) / 3.0).clamp(0.0, 1.0))
                amt = torch.where(ring_ok, amt, fallback)
                if float(amt.max()) > 1e-6:
                    patch = (patch + (patch_low - patch) * amt).clamp(0, 1)
...
def _face_rects_from_transform(transform, n):
    rects = list(transform.get("face_rect") or [])
    if len(rects) >= n:
        return rects[:n]
    cw, ch = transform.get("canvas", (512, 512))
    cf = float(transform.get("crop_factor", 2.5)) or 2.5
    fw = float(cw) / cf
    fh = float(ch) / cf
    fallback = (float(cw) * 0.5 - fw * 0.5, float(ch) * 0.5 - fh * 0.5, fw, fh)
    if not rects:
        return [fallback] * n
    return rects + [rects[-1]] * (n - len(rects))


def apply_face_token_noise_mask(av_latent, transform, strength, dilation=FACE_INPAINT_DILATION):
    """Set NestedTensor noise_mask: face tokens * strength, audio zeros."""
    samples = av_latent.get("samples")
    ...
    members = list(samples.unbind())
    video = members[0]
    n = int(transform.get("frames") or len(transform.get("boxes") or []))
    if n <= 0:
        n = int(np.asarray(strength).shape[0])
    cw, ch = transform["canvas"]
    rects = _face_rects_from_transform(transform, n)
    detected = transform.get("detected")
    vmask_np = face_token_video_mask(
        ch, cw, rects, int(video.shape[-3]), int(video.shape[-2]), int(video.shape[-1]),
        strength=strength, detected=detected, dilation=dilation,
    )
    audio_shape = tuple(members[1].shape) if len(members) > 1 else (1, 0)
    video_np, audio_np = pack_av_noise_mask(vmask_np, audio_shape)
    vmask = torch.from_numpy(video_np).to(device=video.device, dtype=torch.float32)
    vmask = vmask.expand(video.shape[0], video.shape[1], -1, -1, -1).contiguous()
    ...
    if prev is not None and (isinstance(prev, comfy.nested_tensor.NestedTensor)
        ...
        if len(pm) > 1:
            pm[1] = torch.zeros_like(pm[1])
        new_mask = comfy.nested_tensor.NestedTensor(tuple(pm))
    else:
        audio_zero = torch.from_numpy(audio_np).to(device=video.device, dtype=members[1].dtype) \
    ...
    coverage = float((vmask_np > 0.5).mean()) if vmask_np.size else 0.0
    return out, coverage


class H3PerFrameDenoise:
    """Scale denoise on face tokens only, inversely to how big the face is.

    The crop still gives H3 context, but noise_mask is 1 on the full-face ellipse
    (token-grid snapped) and 0 on the rest of the crop and on audio. High scheduler
    denoise can rewrite the face without inventing a new head/shoulders.

    Place this AFTER MiniMaxH3NativeAudioLock so the audio-side zeros stay frozen.
    Granularity is one H3 latent frame (17 pixel frames per 5 latents, ~3.4 frames).
    """
    ...
    CATEGORY = "MiniMax H3/Face Refine"
    DESCRIPTION = (
        "Face-token inpaint: denoise the full face inside the crop, freeze the rest "
        "and the audio. Strength still scales with face size."
    )
    ...
    def run(self, av_latent, transform, strength_small_face, strength_large_face,
        ...
        if transform.get("face_h"):
            face = np.array(transform["face_h"], dtype=np.float64)
        else:
        ...
        if transform.get("weights"):
            w = np.asarray(transform["weights"], dtype=np.float64)
            if w.size < len(strength):
                w = np.pad(w, (0, len(strength) - w.size), mode="edge")
            strength = strength * np.clip(w[: len(strength)], 0.0, 1.0)

        out, coverage = apply_face_token_noise_mask(av_latent, transform, strength)
        report = (
            f"face-token inpaint: face {face.min():.0f}-{face.max():.0f}px, ramp "
            ...
            f"face coverage {coverage * 100:.1f}%"
        )
