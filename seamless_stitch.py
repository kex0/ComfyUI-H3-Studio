...
# Packed opening is 22 frames (~0.9 s). Fade generated→original across most of
# it, then hold a short stretch of clip-1 original so the player-loop is clean.
LOOP_AUDIO_ORIGINAL_HOLD_FRAMES = 4
...
def _match_frame_luma_to_neighbors(frame: torch.Tensor, prev: torch.Tensor, nxt: torch.Tensor) -> torch.Tensor:
    """Dim a single bright frame to the neighbor-average luma. Pixels are not mixed."""
    def luma(x: torch.Tensor) -> float:
        return float(_rgb_luminance(x.unsqueeze(0)).mean().item())

    current = luma(frame)
    target = 0.5 * luma(prev) + 0.5 * luma(nxt)
    if current <= target or current < 1e-6:
        return frame
    return apply_rgb_gain(frame.unsqueeze(0), target / current)[0]
...
def close_loop_av(images: torch.Tensor, audio: dict | None,
                  video_crossfade_frames: int = 0, audio_crossfade_ms: float = 15.0,
                  ) -> tuple[torch.Tensor, dict | None, dict[str, int]]:
    """Overlap-add the file tail with the real opening so a player-loop is continuous.

    When ``video_crossfade_frames`` is the packed clip-1 opening length, the last
    C frames (Loop-generated opening) are cosine-blended with the first C frames
    (clip 1's real opening). Those C frames are dropped from the start so the
    action is not played twice. The H3 phase-0 1-frame at the packed-window
    entry is luma-matched to its neighbors (no skipped motion, no pixel mix).
    Audio does not overlap-add the two takes (that phases). It fades from the
    generated tail into clip 1's original opening across most of that window,
    then holds the original so the player-loop lands on the same clean recording
    as the file start. ``audio_crossfade_ms`` is only used when C is 0.
    """
    stats = {"loop_video_frames": 0, "loop_audio_samples": 0}
    if images is None or int(images.shape[0]) < 2:
        return images, audio, stats

    n = int(images.shape[0])
    c = max(0, int(video_crossfade_frames))
    if c > 0 and n >= 2 * c + 1:
        work = images.clone()
        entry = n - c
        if c >= 2 and entry >= 1:
            work[entry] = _match_frame_luma_to_neighbors(
                work[entry], work[entry - 1], work[entry + 1],
            )
            if entry >= 2:
                work[entry - 1] = _match_frame_luma_to_neighbors(
                    work[entry - 1], work[entry - 2], work[entry],
                )
        blended = blend_video_overlap(work[-c:].contiguous(), work[:c].contiguous())
        images = torch.cat((work[c:-c], blended), dim=0)
        stats["loop_video_frames"] = c

    waveform, sr = _as_audio_waveform(audio) if audio is not None else (None, None)
    if waveform is None or not sr:
        return images, audio, stats

    requested = max(0, int(round(float(audio_crossfade_ms) / 1000.0 * sr)))
    if stats["loop_video_frames"]:
        drop = max(1, int(round(float(stats["loop_video_frames"]) / float(FPS) * sr)))
        if int(waveform.shape[-1]) >= 2 * drop + 1:
            last_c = waveform[..., -drop:].contiguous()
            first_c = waveform[..., :drop].contiguous()
            last_c, first_c = _match_audio_channels(last_c, first_c)
            hold = min(
                max(1, int(round(LOOP_AUDIO_ORIGINAL_HOLD_FRAMES / float(FPS) * sr))),
                max(1, drop // 4),
            )
            fade = drop - hold
            if fade <= 0:
                ...
                head = blend_audio_overlap(last_c[..., :fade], first_c[..., :fade])
                tail = torch.cat((head, first_c[..., fade:].contiguous()), dim=-1)
            waveform = torch.cat((waveform[..., drop:-drop], tail), dim=-1)
            audio = {"waveform": waveform, "sample_rate": sr}
            stats["loop_audio_samples"] = drop
        return images, audio, stats

    fade = requested
    if fade > 0 and int(waveform.shape[-1]) >= 2 * fade + 1:
        tail = waveform[..., -fade:].contiguous()
        head = waveform[..., :fade].contiguous()
        waveform = waveform.clone()
        waveform[..., -fade:] = blend_audio_overlap(tail, head)
        audio = {"waveform": waveform, "sample_rate": sr}
        stats["loop_audio_samples"] = fade
