...
The filler does not pick mic, dance, or worlds. After fill, rewrite `summary` / `detailed_description` as the music-video direction: timed cuts, committed per-shot business (wireless mic at the mouth **or** dancing with empty hands — never both in one shot), new environments only after `the camera cuts`. Never write `The space becomes`. Never write `sometimes`.
